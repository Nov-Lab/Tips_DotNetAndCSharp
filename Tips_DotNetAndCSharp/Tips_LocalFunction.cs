...
// @(h)Tips_LocalFunction.cs ver 0.51b( '24.04.27 Nov-Lab ) その他  ：コメント整理
...
namespace Tips_DotNetAndCSharp
{
    public partial class Tips
    {
        ...
        /// <summary>
        /// 【C#の機能：ローカル関数】<br/>
        /// ・ローカル関数は、メソッドやプロパティーアクセサーなど、親メンバーの中に定義する内部メソッドです。<br/>
        /// ・ローカル関数は、そのローカル関数が定義されている親メンバーからのみ呼び出すことができます。<br/>
        /// ・そのため、無関係なメソッドから誤って呼び出してしまう心配がなく、親メンバーの内部で完結することができます。<br/>
        /// ・ローカル関数は、親メンバーで宣言されているローカル変数を直接読み書きすることができます。<br/>
        /// ・そのため、引数や戻り値を介さずに情報をやり取りすることができます。<br/>
        /// </summary>
        //--------------------------------------------------------------------------------
        ...
        public static void Tips_LocalFunction()
        {
            ...
            {
                seqNo++;                                                //// 連番をインクリメントする <- 親メンバーで宣言されているローカル変数を直接読み書きするサンプル
